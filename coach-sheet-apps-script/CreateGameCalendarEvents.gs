...
function createGameCalendarEvents() {
  ...
  try {
    ...
    const isGameEventOurs = function (e) {
      ...
      return title.indexOf(GAME_EVENT_TITLE_PREFIX) === 0 || title === GAME_WARMUP_TITLE;
    };
...
/**
 * Get game event specs from Game Info sheet using getDatesFromInfoSheet and exact column names.
...
function getGameEventSpecsFromSheet(ss, infoSheet) {
  ...
  const colLocation = col(GAME_INFO_COLUMNS.LOCATION);
  const colLocationUrl = col(GAME_INFO_COLUMNS.LOCATION_URL);
  ...
  const lastCol = Math.max(
    ...
    colLocation >= 0 ? colLocation + 1 : 0,
    colLocationUrl >= 0 ? colLocationUrl + 1 : 0,
  ...
  dateInfos.forEach(function (dateInfo) {
    ...
    // Only when game start time is TBD: create 0-minute event at 9am (empty or literal "TBD")
    ...
    if (!gameStartTime) {
      // Unparseable game start (e.g. "TBA") -> treat as TBD and create 0-min 9am event
      const tbdStart = new Date(dateObj.getFullYear(), dateObj.getMonth(), dateObj.getDate(), TBD_DEFAULT_TIME_HOUR, TBD_DEFAULT_TIME_MINUTE, 0);
      specs.push({
        title: GAME_TBD_TITLE,
        startTime: tbdStart,
    ...
    if (locationUrl) descriptionParts.push('Location URL: ' + locationUrl);
    if (gameNote) descriptionParts.push(gameNote);
    ...
    if (warmupVal && (typeof warmupVal === 'string' ? warmupVal.trim() : warmupVal)) {
      ...
      if (warmupStart && warmupStart < gameStartTime) {
        const warmupDescription = locationUrl ? locationUrl : '';
        specs.push({
