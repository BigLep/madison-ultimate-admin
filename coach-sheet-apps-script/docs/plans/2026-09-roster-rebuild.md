...
- 2026-09-07, later (Steven's request): every Roster Boolean now reads TRUE for "all is well" and FALSE for "needs attention". Gender Special Attention renamed **Gender Default Handling** with the formula inverted; Media Opt-Out renamed **Media OK** with the formula inverted (TRUE unless the Signups Media Opt-Out is true).
- 2026-09-07, later (Steven's request): new derived Boolean column **Has Playing Experience** inserted between Returning and Include In Generated Rosters, for pivot-table analysis. Initially a regex keyword heuristic over the Signups Playing Experience text (chosen over an `=AI()` classification, which writes static values instead of a live self-maintaining formula); superseded the same day, see below.
- 2026-09-07, later still (Steven's request): replaced the regex heuristic with a coach-authored **Number of Past Seasons** column in Extra Player Info (a count Claude read off each player's Signups Playing Experience text, e.g. "3 years at Lafayette" to `3`; left blank where the text gave no usable count, including most Returning players whose text didn't mention a number), plus a read-only **Signup Playing Experience** XLOOKUP column next to it in Extra Player Info so a coach can spot-check the count against the source text. The Roster's **Number of Past Seasons** now passes that through directly, and **Has Playing Experience** simplified to `Returning OR Number of Past Seasons > 0` instead of parsing text itself (40 columns becomes 42).
- 2026-09-09 (Steven's request): new passthrough **Tryout Group** column appended at the end of the Roster. Steven had already added **Tryout Group** and **Signup Grade** directly to the live Extra Player Info sheet (typed during tryouts, ahead of the code); `EXTRA_PLAYER_INFO_HEADERS`/`EXTRA_PLAYER_INFO_LETTERS` in Code.gs now include both in their live order so `Include In Generated Rosters` keeps resolving to the right column. Only Tryout Group was added to the Roster, since that's what was asked for; Signup Grade is mapped but not yet on the Roster (42 columns becomes 43).
- 2026-09-09, later (Steven's request): new **Tryout ID** column in Extra Player Info (right after Tryout Group) and passed through to the Roster right after Tryout Group. A fixed, three-digit id for referencing a Player on tryout printouts: first digit is Grade (6/7/8, from the Roster's Grade column per Steven's choice), remaining two digits are a sequence within that Grade and gender assigned alphabetically by Full Name, 00-49 for Bx and 50-99 for Gx. Claude generated the initial values for all 110 Players with a Tryout Group set and a resolvable Grade/Gender Identification (grade 6 Bx 600-646, grade 6 Gx 650-671, grade 7 Bx 700-710, grade 7 Gx 750-762, grade 8 Bx 800-808, grade 8 Gx 850-857); left blank for 5 Players with no Grade and/or Gender Identification yet (Mateo Andrade, Callum Leonard, Emmy Madison, Isaac Pistolesi, Kirby Wilhelm) for Steven to assign once their Signups data is complete. A coach-authored fixed value, not a live formula, so printed ids never shift (43 columns becomes 44).
- 2026-09-11 (Steven's request): new **Assign Tryout IDs** menu item (`assignTryoutIds` in ExtraPlayerInfo.gs) so new Players added to tryouts after the initial batch get a Tryout ID without Claude hand-computing it again. Fills blank Tryout ID cells for Players with a Tryout Group set, using the Roster's Grade and Gender Identification; never renumbers an existing id, extends a Grade/gender bucket at the next offset after its current highest, and orders several new Players landing in the same bucket at once by Full Name. Reports and skips Players whose Grade/Gender Identification isn't resolved yet.
