...
- 2026-09-07, later (Steven's request): new derived Boolean column **Has Playing Experience** inserted between Returning and Include In Generated Rosters, for pivot-table analysis. TRUE when Returning is TRUE, or when the Signups Playing Experience text reads as prior organized play (a season/year/grade, a team/club/camp/clinic, Ultimate/Frisbee/DiscNW by name, or a bare number). FALSE for blank text, an explicit no/none answer, or "new to Ultimate"/"first season" phrasing, even alongside other sports. A regex keyword heuristic rather than an `=AI()` classification, so it stays a live formula that self-maintains for new signups instead of needing manual per-batch re-generation; validated against the ~60 non-blank Fall 2026 Playing Experience answers before landing (40 columns becomes 41).
