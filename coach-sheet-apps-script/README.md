...
Full Name (Preferred First Name followed by Last Name, derived in the Roster) is the human-readable key every Generated Roster and email list uses to refer to a Player. The availability sheets are keyed by `PlayerID` in column A, the only per-player value Build Practice/Game Availability types; their Full Name, Grade, and Gender Identification cells are per-row Roster lookup formulas on that PlayerID (ADR 0004), so a family editing their preferred name can neither orphan their row nor leave a stale name behind. The roster prep sheets XLOOKUP into the availability sheets by Full Name, finding that column by header rather than assuming column A. The player portal never matches on Full Name: it reads and writes availability cells by PlayerID.
