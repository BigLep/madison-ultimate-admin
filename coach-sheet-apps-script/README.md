...
### Sheet Builders
- **Build Practice Roster** - Create roster with practice availability columns. Base columns are `#`, Full Name, Team, Gender, Grade, then a hidden `PlayerID` column: the PlayerID is the row's key (the only typed value), and every other cell is a per-row lookup on it (Roster for Full Name/Team/Gender/Grade, Practice Availability and Game Availability for the date columns). Unhide column F if you need to see the key.
- **Build Game Roster Prep Sheet** - Create game day roster (coach or parent view); like the practice roster, each row is keyed by a hidden `PlayerID` column (after Grade in the coach view, last in the parent view) and every other cell is a lookup on it. If **Game Info** has multiple rows on the **same calendar date**, the prep sheet includes **all** of those games (see [Multiple events on one calendar day](#multiple-events-on-the-same-calendar-day-double-headers)).
