...
- DONE 2026-09-07: purged five named test rows from the portal's Signups sheet (MobileTest, Inspect, and GrillTest TestCleared; RefreshTest TestFixture; CleanVerify FixTest) and their five orphaned Extra Player Info rows. Signups, Roster, and Extra Player Info all sit at 75 rows. Left for Steven to judge: "Bob Larson" (g94df, no contact info, created among the fixtures), "A E" (qk4mx), and "Big Loep" (tt3t7) look like test signups but are not named as such.
...
- DONE 2026-09-07: deleted photo-mapper/ and its README mention (commit "chore: delete photo-mapper"). The untracked photo-mapper/backend/.google-service-account.json key file was left on disk for Steven to delete or revoke in GCP; nothing in the repo uses it now.
